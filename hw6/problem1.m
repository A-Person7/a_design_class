...
% Notation:
%   I may use mat to abreviate material
...
%% Material Properties
% First things first--transform ultimate strength to operating temperature
% Operating temperature
T_op = 150; % [C]

% Use formula given in slides for temperature in Celcius
S_T_by_S_RT = 0.99 + 5.9*10^(-4)*T_op - 2.1 * 10^(-6)*T_op^2;
% Sanity check manually with Table 6-4 values

% Table A-20 in Shigley, transformed for temp
S_ut_st = 470 * S_T_by_S_RT; % [MPa]
% Table A-24
S_ut_al = 248 * S_T_by_S_RT; % [MPa]

% Assume high cycle fatigue because, uhm, uh, hmmm
...
elseif (S_ut_al > 1400) % 1400 is in [MPa]
...
% Already transformed temperature earlier
k_d_al = 1;
k_d_st = 1;
...
% No miscellanous effects are present
k_f_al = 1;
k_f_st = 1;
...
% Whoops, ignore everything after this line
...
% 
% % See Figure 6-18
% % Reading the text, this is applicable for when S_e = 0.5*S_y 
% %   However, we're going to have to make some CRUDE approximations here as I don't see an 
% %   applicable chart for aluminum
% % use temperature-transformed S_ut value
% % 255.7 MPa ~ 37.1 ksi, which is off the chart, therefore use f = 0.9
% f_al = 0.9;
% 
% % Need to be careful, Shigley, slides treat log as layman/engineers (log_10), but MATLAB
% %   treats it as a mathmatician might (natural log), need to be explicit
% % Personally, log being log_10 makes more sense, because ln is shorter anyways
% b = 1/(3 - log10(N_max_al)) * log10(f_al * S_ut_al / S_f_5E8_al);
% a = 10^((log10(N_max_al) * log10(f_al * S_ut_al) - 3*log10(S_f_5E8_al))/(log10(N_max_al)-3));
% 
% % Steel
% % Figure 6-18, use temperature-transformed S_ut value
% % "If you squint, it's just slightly less than 0.9..."
% f_st = 0.899;
% 
% a_st = (f_st * S_ut_st)^2 / (S_e_st);
% b_st = -1/3 * log10(f_st*S_ut_st / S_e_st);
% 
% N_al = (S_f_5E8_al/a_al) ^ (1/b_al); % [cycles]
% N_st = (S_e_st/a_st) ^ (1/b_st); % [cycles]
% 
% fprintf("\nN_st  = %s cycles.\n", addComma(N_al));
% fprintf(  "N_al  = %s cycles.\n", addComma(N_st));
% 
% % Use Java formatting code to get nicer looking numbers
